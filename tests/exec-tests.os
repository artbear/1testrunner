...
Процедура ТестДолжен_ПроверитьВыводПараметризированныхТестов() Экспорт
	...
	// FIXME 1testrunner возвращает отрицательный код возврата для статуса "Не выполнялся", 
	// что превращается в 255, т.к. коды возврата - это неотрицательное значение.
	КодВозвратаНеВыполнялся = 255; 

	ЯВыполняюКомандуПродуктаCПередачейПараметров("-show", ПутьФайлаТеста, КодВозвратаНеВыполнялся, СтрВывод);
...
Процедура ЯВыполняюКомандуПродуктаCПередачейПараметров(Знач Команда, Знач ПараметрыКоманды,Знач ОжидаемыйКодВозврата=0, ТекстВывода="")
	...
	КодВозврата = ВыполнитьПроцесс(СтрокаКоманды, ТекстВывода);
