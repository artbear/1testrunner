...
Функция ОбработатьПараметрыЗапуска(МассивПараметров)
	...
	УспешноЗагрузили = ЗагрузитьТесты();

	Возврат УспешноЗагрузили;
КонецФункции
...
Процедура ВыполнитьВсеТесты()
	Если НаборТестов.Количество() > 0 Тогда
		...
		Если СоздаватьОтчетТестированияВФорматеJUnitXML Тогда
			ЗаписьXML = НачатьЗаписьВФайлОтчетаТестированияВФорматеJUnitXML();
		КонецЕсли;
...
Функция НачатьЗаписьВФайлОтчетаТестированияВФорматеJUnitXML()
...
//   Статус - СтатусСообщения - <описание параметра>
//
