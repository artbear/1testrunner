...
//
Функция ТестироватьФайл(Знач ФайлТеста, Знач ЛогФайлJUnit = Неопределено, Знач ЛогФайлGExec = Неопределено
	, Знач НаименованиеТестаДляЗапуска = Неопределено, Знач НомерТестаДляЗапуска = Неопределено) Экспорт
	
	ПутьФайлаТеста = ФайлТеста.ПолноеИмя;
	УстановитьПутьЛогФайлаJUnit(ЛогФайлJUnit);
	УстановитьПутьЛогФайлаGExec(ЛогФайлGExec);
...
//   Строка - Результат тестирования
//
Функция ТестироватьКаталог(Знач КаталогТестов, Знач ЛогФайлJUnit = Неопределено, Знач ЛогФайлGExec = Неопределено) Экспорт
...
Функция ПолучитьПараметрыЗапуска(МассивПараметров)
	Перем ПутьЛогФайлаJU;
	Перем ПутьЛогФайлаGE;
	...
	ПутьЛогФайлаJU = Неопределено;
	ПутьЛогФайлаGE = Неопределено;
	...
	Если КомандаЗапуска = СтруктураПараметровЗапуска.Запустить Тогда
		...
		Если МассивПараметров.Количество() > НомерОчередногоПараметра Тогда
			...
			Если НРег(ИД_Теста) <> СтруктураПараметровЗапуска.Режим_ПутьЛогФайла И НРег(ИД_Теста) <> СтруктураПараметровЗапуска.Режим_ПутьЛогФайлаGE Тогда
	...
	Если МассивПараметров.Количество() > НомерОчередногоПараметра и (КомандаЗапуска = СтруктураПараметровЗапуска.Запустить или КомандаЗапуска = СтруктураПараметровЗапуска.ЗапуститьКаталог ) Тогда
		...
		Если Режим = СтруктураПараметровЗапуска.Режим_ПутьЛогФайла Тогда
			Если МассивПараметров.Количество() > НомерОчередногоПараметра+1 Тогда
				НомерОчередногоПараметра = НомерОчередногоПараметра+1;
				ПутьЛогФайлаJU = МассивПараметров[НомерОчередногоПараметра];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если МассивПараметров.Количество() > НомерОчередногоПараметра и (КомандаЗапуска = СтруктураПараметровЗапуска.Запустить или КомандаЗапуска = СтруктураПараметровЗапуска.ЗапуститьКаталог ) Тогда
		Режим = НРег(МассивПараметров[НомерОчередногоПараметра]);
		Если Режим = СтруктураПараметровЗапуска.Режим_ПутьЛогФайлаGE Тогда
			Если МассивПараметров.Количество() > НомерОчередногоПараметра+1 Тогда
				НомерОчередногоПараметра = НомерОчередногоПараметра+1;
				ПутьЛогФайлаGE = МассивПараметров[НомерОчередногоПараметра];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыЗапуска.Вставить("ПутьЛогФайлаJUnit", ПутьЛогФайлаJU);
	ПараметрыЗапуска.Вставить("ПутьЛогФайлаGExec", ПутьЛогФайлаGE);
	...
	Лог.Отладка("ПутьЛогФайлаGExec %1", ПутьЛогФайлаGExec);
...
Функция ОбработатьПараметрыЗапуска(МассивПараметров)
	...
	ПутьЛогФайлаGExec = ПараметрыЗапуска.ПутьЛогФайлаGExec;
	
	Если КомандаЗапуска = СтруктураПараметровЗапуска.Запустить Тогда
		ТестироватьФайл(Новый Файл(ПутьКТестам), Новый Файл(ПутьЛогФайлаJUnit), Новый Файл(ПутьЛогФайлаGExec), НаименованиеТестаДляЗапуска, НомерТестаДляЗапуска);
		Возврат Истина;
	КонецЕсли;
	Если КомандаЗапуска = СтруктураПараметровЗапуска.ЗапуститьКаталог Тогда
		ТестироватьКаталог(Новый Файл(ПутьКТестам), Новый Файл(ПутьЛогФайлаJUnit), Новый Файл(ПутьЛогФайлаGExec));
		Возврат Истина;
	КонецЕсли;
...
Функция СоздатьСтруктуруПараметровЗапуска()
	...
	СтруктураПараметровЗапуска.Вставить("Режим_ПутьЛогФайлаGE", НРег("xddReportPathGE"));
...
Функция ВыполнитьВсеТесты()
	Если НаборТестов.Количество() > 0 Тогда
		...
		СоздаватьОтчетТестированияВФорматеGExecXML = ЗначениеЗаполнено(ПутьЛогФайлаGExec);
		Если СоздаватьОтчетТестированияВФорматеGExecXML Тогда
			ЗаписьGEXML = Неопределено;
			НачатьЗаписьВФайлОтчетаТестированияВФорматеGExecXML(ЗаписьGEXML);
		КонецЕсли;
		...
		КонецЕсли;
		Если СоздаватьОтчетТестированияВФорматеGExecXML Тогда
			ЗавершитьЗаписьВФайлОтчетаТестированияВФорматеGExecXML(ЗаписьGEXML, ДатаНачала);
		КонецЕсли;
	КонецЕсли;
КонецФункции
...
Процедура НачатьЗаписьВФайлОтчетаТестированияВФорматеGExecXML(ЗаписьXML)
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку("UTF-8");
	ЗаписьXML.ЗаписатьОбъявлениеXML();
...
Процедура ЗавершитьЗаписьВФайлОтчетаТестированияВФорматеGExecXML(ЗаписьXML, ДатаНачала)
	...
	Для Каждого ОписаниеТеста Из НаборТестов Цикл
		...
		КонецЕсли;
		ЗаполнитьРезультатТестовогоСлучаяGE(ЗаписьXML, ОписаниеТеста, НаборОшибок, НаборНереализованныхТестов);
	КонецЦикла;	
	...
	ПутьОтчетаВФорматеGexecxml = Новый Файл(ПутьФайлаОтчетаТестированияВФорматеGExecXML()+"/"+ФайлТеста.Имя+"_ge.xml");
	
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.ОткрытьФайл(ПутьОтчетаВФорматеGExecxml.ПолноеИмя);
	ЗаписьXML.ЗаписатьБезОбработки(СтрокаХМЛ);// таким образом файл будет записан всего один раз, и не будет проблем с обработкой на билд-сервере TeamCity
	ЗаписьXML.Закрыть();
	Сообщить(" ");
	Сообщить("Путь к лог-файлу проверки в формате Generic <"+ПутьОтчетаВФорматеGExecxml.ПолноеИмя+">");
	
КонецПроцедуры

Процедура ЗаполнитьРезультатТестовогоСлучаяGE(ЗаписьXML, ОписаниеТеста, НаборОшибок, НаборНереализованныхТестов)
...
Функция ПутьФайлаОтчетаТестированияВФорматеGExecXML()
	Возврат ?(ЗначениеЗаполнено(ПутьЛогФайлаGExec), ПутьЛогФайлаGExec, ТекущийКаталог());
КонецФункции
...
Процедура УстановитьПутьЛогФайлаGExec(Знач ЛогФайлGExec)
	Если ЛогФайлGExec = Неопределено Тогда
		ПутьЛогФайлаGExec = Неопределено;
	Иначе
		ПутьЛогФайлаGExec = ЛогФайлGExec.ПолноеИмя;
	КонецЕсли;
КонецПроцедуры
