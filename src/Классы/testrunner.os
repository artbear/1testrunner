	...
	Для Каждого ВариантТеста Из СтрокиАннотаций Цикл
		...
		Для Каждого Параметр Из ПараметрыАннотации Цикл
			Если Параметр.Имя <> Неопределено Тогда
				...
				ВызватьИсключение СтрШаблон("Неверное применение аннотации %1 на методе %2. Ожидаются парамеры-значения, без имен",
					ИмяАннотацииПараметры,
					ИмяМетода);
			КонецЕсли;

			ЗначенияПараметров.Добавить(Параметр.Значение);
			ПредставленияПараметров = ПредставленияПараметров + "," + Строка(Параметр.Значение);
		КонецЦикла;

		Случай = Новый Структура("ИмяТеста,Параметры,Представление", 
			ИмяМетода,
			ЗначенияПараметров,
			СтрШаблон("%1(%2)", ИмяМетода, Сред(ПредставленияПараметров, 2))
		);
