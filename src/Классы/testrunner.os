...
//
Функция ТестироватьФайл(Знач ФайлТеста, Знач ЛогФайлJUnit = Неопределено,
...
Функция ПолучитьПараметрыЗапуска(МассивПараметров) Экспорт
	...
	ИначеЕсли КомандаЗапуска = СтруктураПараметровЗапуска.Запустить
			Или КомандаЗапуска = СтруктураПараметровЗапуска.ЗапуститьКаталог Тогда
		НомерПараметраПутьКТестам = 1;
	...
	Если МассивПараметров.Количество() > НомерОчередногоПараметра
			И (КомандаЗапуска = СтруктураПараметровЗапуска.Запустить
				ИЛИ КомандаЗапуска = СтруктураПараметровЗапуска.ЗапуститьКаталог ) Тогда
...
Функция ОбработатьПараметрыЗапуска(МассивПараметров)
	...
	Если КомандаЗапуска = СтруктураПараметровЗапуска.Запустить Тогда
		ТестироватьФайл(Новый Файл(ПутьКТестам), Новый Файл(ПутьЛогФайлаJUnit),
			НаименованиеТестаДляЗапуска, НомерТестаДляЗапуска);
		Возврат Истина;
	КонецЕсли;
...
Функция ЗагрузитьТесты()
	...
	Для Каждого ПутьТеста Из Пути Цикл
		...
		Иначе
			...
			ИмяКлассаТеста = СтрЗаменить(Файл.ИмяБезРасширения, "-", "")
			...
			МассивТестовыхСлучаев = ПолучитьТестовыеСлучаи(Тип(ИмяКлассаТеста), ПолноеИмяТестовогоСлучая);
			Если МассивТестовыхСлучаев = Неопределено Тогда
				Продолжить;
			КонецЕсли;

			Тест = ТестОбъект(Тип(ИмяКлассаТеста));
...
Процедура ЗаполнитьРезультатТестовогоСлучая(ЗаписьXML, ОписаниеТеста, НаборОшибок, НаборНереализованныхТестов)
	...
	Если СтруктураОшибки <> Неопределено Тогда
		СтрокаРезультат = ?(СтруктураОшибки.СостояниеВыполнения = ЗначенияСостоянияТестов.Сломался,
							"failure", "skipped");
...
Процедура ЗавершитьЗаписьВФайлОтчетаТестированияВФорматеGExecXML(ЗаписьXML, ДатаНачала)
	...
	Для Каждого ОписаниеТеста Из НаборТестов Цикл
		ПредыдущееИмяТеста = ФС.ОтносительныйПуть(КаталогПроекта, ОписаниеТеста.ПолноеИмя, "/");
		Если НЕ ИмяТекущегоТеста = ПредыдущееИмяТеста Тогда
			Если НЕ ИмяТекущегоТеста = "" Тогда
				ЗаписьXML.ЗаписатьКонецЭлемента();
			КонецЕсли;
			ИмяТекущегоТеста = ФС.ОтносительныйПуть(КаталогПроекта, ОписаниеТеста.ПолноеИмя, "/");
	...
	ПутьОтчетаВФорматеJUnitxml = Новый Файл(ПутьФайлаОтчетаТестированияВФорматеJUnitXML()
		+ "/" + ФайлТеста.Имя + ".xml");
...
Процедура ЗаполнитьРезультатТестовогоСлучаяGExec(ЗаписьXML, ОписаниеТеста, НаборОшибок, НаборНереализованныхТестов)
	...
	Если СтруктураОшибки <> Неопределено Тогда
		...
		КонецЕсли;
		ЗаписьXML.ЗаписатьАтрибут("message", "(" + СтруктураОшибки.ИнфоОшибки.НомерСтроки + "): "
			+ СокрЛП(СтруктураОшибки.ИнфоОшибки.ИсходнаяСтрока));
...
Функция ПолучитьТестовыеСлучаи(ТестТип, ПолноеИмяОбъекта)

	Попытка

		МассивТестовыхСлучаев = ПрочитатьТестовыеСлучаиОбъекта(ТестТип, ЭтотОбъект);
...
//
//  Параметры:
// 		НовыйФорматЛогФайла		- Число		- формат лог файла из метода ФорматыЛогФайла
//
...
//
Процедура ПрерватьТест(ТекстОшибки) Экспорт
...
//
// Параметры:
//  ТестТип - Тип - Тип тестового сценария
//
...
//
Функция ПрочитатьТестовыеСлучаиОбъекта(Знач ТестТип, Знач МенеджерТестирования)
	МассивТестовыхСлучаев = ПрочитатьТестовыеСлучаиОбъектаВызовомМетода(ТестТип, МенеджерТестирования);
	Если Неопределено <> МассивТестовыхСлучаев Тогда
		Возврат МассивТестовыхСлучаев;
	КонецЕсли;

	МассивТестовыхСлучаев = НайтиАннотированныеТесты(ТестТип);
...
//   Массив   - массив строк-имен методов тестового сценария
//
Функция ПрочитатьТестовыеСлучаиОбъектаВызовомМетода(Знач ТестТип, Знач МенеджерТестирования)

	Если НЕ Рефлектор.МетодСуществует(ТестТип, "ПолучитьСписокТестов") Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	МассивТестовыхСлучаев = ТестОбъект(ТестТип).ПолучитьСписокТестов(МенеджерТестирования);
	Если ТипЗнч(МассивТестовыхСлучаев) = Тип("Массив") Тогда
		Для каждого ТестовыйСлучай Из МассивТестовыхСлучаев Цикл
...
Функция НайтиАннотированныеТесты(Знач ТестТип)

	ТаблицаМетодов = Рефлектор.ПолучитьТаблицуМетодов(ТестТип);
	Если ТаблицаМетодов.Колонки.Найти("Аннотации") = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ТестОбъект(Знач ТестТип)

	Объект = КешТестОбъект.Получить(ТестТип);

	Если Объект = Неопределено Тогда
		Объект = Новый(ТестТип);
		КешТестОбъект.Вставить(ТестТип, Объект);
	КонецЕсли;
...
Процедура Инициализация()
	...
	ИспользоватьСборСтатистикиСкриптовOnescript = Ложь;
	КаталогСбораСтатистики = Неопределено;
	КешТестОбъект = Новый Соответствие();
